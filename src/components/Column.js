...
const Column = (props) => {
  ...
  return (
    <div className='flex flex-col gap-4   '>
        ...
        {props.board[props.status].map((task,index)=> <Task key={Math.random()} taskIndex={index} statusColumn={props.status} showTaskDetails={props.showTaskDetails} taskDetails={task}/>)}
    </div>
  )
}
